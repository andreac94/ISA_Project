...
with open('inputs23', 'r') as i, open('results.txt', 'r') as f, open('comparison','w') as o:
    ...
    while True:
        ...
        A_dec = long(int(sim_A_bin_fraction, 2)) * 2**29

        # Second input
        sim_B_bin = i.readline()
        if not sim_B_bin: break
        sim_B_bin_fraction  = '1' + sim_B_bin
        B_dec = long(int(sim_B_bin_fraction, 2)) * 2**3

        # Operation Result
        sim_result_bin  = f.readline()
        sim_pos_q_bin   = f.readline()
        sim_neg_q_bin   = f.readline()
        sim_remaind_bin = f.readline()
        sim_result_dec  = long(int(sim_result_bin, 2))

        result_dec = A_dec / B_dec

        # Convert the result binary
        temp_result_bin = format((int(result_dec )), 'b')
     ...
     # A_dec, B_dec, '', sim_remaind_bin.rstrip()))
        o.write(
                '{:5s}{:35s}{:<15}{:<28s}{:<24s}\n{:5s}{:35s}{:<15}{:<28}{:<24}\n{:5s}{:28s}\n{:5s}{:28s}\n{:5s}{:28s}\n   \n'.format(
                    ...
                    validation, result_bin, result_dec, A_dec, B_dec,'POS',
                    sim_pos_q_bin.rstrip(), 'NEG', sim_neg_q_bin.rstrip(),
                    'Rem', sim_remaind_bin.rstrip()     ))
